...
func allow_mount(disk: DADisk, context: UnsafeMutableRawPointer?) -> Unmanaged<DADissenter>?
{
    ...
    guard let uuid = description[kDADiskDescriptionVolumeUUIDKey] as! CFUUID? else {
    ...
    if (are_uuid_equal(uuidBytes, MSI_MONITOR_UUID)) {
        ...
        let dissenter = DADissenterCreate(kCFAllocatorDefault, Int32(kDAReturnExclusiveAccess), "It's mine!" as CFString);
